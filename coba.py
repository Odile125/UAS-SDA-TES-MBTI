...
pertanyaan_per_dimensi = {
    "I": ["Saya lebih suka menyendiri di waktu luang.", "Saya merasa capek setelah banyak interaksi sosial."],
    "E": ["Saya senang bertemu orang baru.", "Saya merasa bersemangat setelah berbicara dengan banyak orang.", "Saya cenderung berbicara daripada mendengarkan."],
    "S": ["Saya fokus pada fakta, bukan kemungkinan.", "Saya lebih suka pengalaman langsung daripada teori."],
    "N": ["Saya suka membayangkan masa depan.", "Saya tertarik pada ide-ide abstrak.", "Saya sering memikirkan berbagai kemungkinan."],
    "T": ["Saya membuat keputusan berdasarkan logika, bukan perasaan.", "Saya suka berargumen untuk menemukan kebenaran."],
    "F": ["Saya mempertimbangkan perasaan orang lain dalam keputusan.", "Saya lebih peduli hubungan daripada kebenaran logis.", "Saya merasa tidak nyaman jika membuat orang lain kecewa."],
    "J": ["Saya suka membuat rencana dan mengikuti jadwal.", "Saya merasa gelisah jika rencana berubah."],
    "P": ["Saya lebih suka fleksibel daripada membuat rencana.", "Saya santai menghadapi perubahan rencana.", "Saya sering menyelesaikan tugas di menit terakhir."]
}
...
# Fungsi prediksi MBTI berdasarkan pohon keputusan
...
# Visualisasi pohon MBTI menggunakan graphviz
def visualisasi_pohon(pohon):
    dot = Digraph()
    
    def tambah_node(node, id_node):
        if node is None:
            return
        label = node.root if node.nilai is None else node.nilai
        dot.node(str(id_node), label)
        
        if node.right:
            id_kanan = f"{id_node}1"
            tambah_node(node.right, id_kanan)
            dot.edge(str(id_node), id_kanan, label="Ya/1")
        
        if node.left:
            id_kiri = f"{id_node}0"
            tambah_node(node.left, id_kiri)
            dot.edge(str(id_node), id_kiri, label="Tidak/0")
    
    tambah_node(pohon, "root")
    return dot

if __name__ == "__main__":
    tes_mbti()

    # Tampilkan dan simpan pohon sebagai file gambar
    dot = visualisasi_pohon(root_pohon)
    dot.render("pohon_mbti", format="png", view=True)
